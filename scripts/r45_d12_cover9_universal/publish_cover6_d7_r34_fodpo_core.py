#!/usr/bin/env python3
"""Publish and verify the exact FoDPO compact LRAT core in the repository.
...
TRACKED_DIRECTORY = HERE / "master7_r34_fodpo_core"
TRACKED_REPORT = HERE / "MASTER7_R34_FODPO_LRAT_CORE_V1.json"
LEAN_MODULE = (
    REPOSITORY
    / "vendor"
    / "lrat-catcher"
    / "LRATCatcher"
    / "Tests"
    / "R44Cover6Master7R34FoDPOCore.lean"
)
...
EXTERNAL_CORE_DEFAULT = Path(
    r"S:\CodexResearchCache\ramsey-formal\lrat-work\r45-cover6-master7-r34-fodpo-core-v1"
)
...
CATALOGUE_INDEX = 6
ICNF_INDEX = 6
INCREMENTAL_POSITION = 7
CORE_CLAUSES = 7_686
SOURCE_CLAUSES = 4_312_440
SOURCE_VARIABLES = 66

CORE_CNF_NAME = "cover6_closed_f7_r34_i7_FoDPO_core.cnf"
CORE_LRAT_NAME = "cover6_closed_f7_r34_i7_FoDPO_core.lrat"
...
QUALIFIED_REPLAY_THEOREM = (
    "LRATCatcher.Tests.Cover6D7R34FoDPOCoreReplay."
    "cover6_d7_r34_fodpo_core_unsat"
)
...
EXACT_EXTERNAL_ARTIFACTS = {
    ...
    CORE_LRAT_NAME: {
        "bytes": 1_127_442,
        "lines": 22_791,
        "sha256": "9947702DC26FF5F97ABE7279014F309073196CE1F225CAA677EE496BA447E1C1",
    },
    MAPPING_NAME: {
        "bytes": 596_608,
        "lines": 7_687,
        "sha256": "D1BE4CB5C4F2DC989A70C58E95A386F360FCF2D5351108545F435BA8B2EE62D8",
    },
    REDUCTION_NAME: {
        "bytes": 6_443,
        "lines": 218,
        "sha256": "F414A5545F887FF4DF62376C1ED6DDBC87A701D435AA6B5CEFC81B2A5B315C34",
    },
    REPLAY_NAME: {
        "bytes": 548,
        "lines": 12,
        "sha256": "23D3214917389D37F53C84D0814C39477E00ECCCEB9D2358186F4212C46092AB",
    },
}
EXTERNAL_MANIFEST = {
    "bytes": 13_850,
    "lines": 411,
    "sha256": "D64864D8F75555BCBE8A4A68239A9FCFD18394C556898B73DDD023A61B5813C2",
}
EXTERNAL_LEAN_LOG = {
    "bytes": 201,
    "lines": 4,
    "sha256": "C403B664A8F560702C49F0A6F374E63DCE4FAB939743EBA4E06F6FF1BA131BB8",
}
...
class FoDPOPublicationError(ValueError):
...
def assert_portable(payload: bytes, label: str) -> None:
    if ABSOLUTE_PATH_PATTERN.search(payload):
        raise FoDPOPublicationError(f"{label} contains an absolute path")
...
def ensure_repo_path(path: Path, label: str) -> None:
    try:
        ...
        raise FoDPOPublicationError(f"{label} is outside the repository") from error
...
def copy_new(source: Path, target: Path, expected: dict[str, object]) -> FileMeta:
    ...
    for field in ("bytes", "lines", "sha256"):
        if getattr(actual, field) != expected[field]:
            raise FoDPOPublicationError(f"copied {target.name} {field} mismatch")
    return actual
...
namespace LRATCatcher.Tests.Cover6D7R34FoDPOCoreReplay

-- Exact frozen-DIMACS core only; no semantic S7 composition here.
lrat_reflect cover6_d7_r34_fodpo_core_unsat
  \"../../scripts/r45_d12_cover9_universal/master7_r34_fodpo_core/{CORE_CNF_NAME}\"
  \"../../scripts/r45_d12_cover9_universal/master7_r34_fodpo_core/{CORE_LRAT_NAME}\"

#print axioms cover6_d7_r34_fodpo_core_unsat

end LRATCatcher.Tests.Cover6D7R34FoDPOCoreReplay
""".encode("utf-8")
...
def read_zero_based_indices(mapping_path: Path) -> list[int]:
    with mapping_path.open("r", encoding="ascii", newline="") as stream:
        ...
        if header != "core_clause_id\tfodpo_clause_id\tclause_sha256\n":
            raise FoDPOPublicationError("FoDPO mapping header changed")
        indices: list[int] = []
        previous = 0
        for expected_core_id, raw in enumerate(stream, start=1):
            fields = raw.rstrip("\n").split("\t")
            if len(fields) != 3:
                raise FoDPOPublicationError("malformed FoDPO mapping row")
            core_id_text, source_id_text, clause_sha = fields
            try:
                core_id = int(core_id_text)
                source_id = int(source_id_text)
            except ValueError as error:
                raise FoDPOPublicationError("non-integer FoDPO mapping id") from error
            if core_id != expected_core_id:
                raise FoDPOPublicationError("FoDPO core ids are not contiguous")
            if not 1 <= source_id <= SOURCE_CLAUSES or source_id <= previous:
                raise FoDPOPublicationError("FoDPO source ids are not strict/in-bounds")
            if SHA256_PATTERN.fullmatch(clause_sha) is None:
                raise FoDPOPublicationError("malformed FoDPO clause hash")
            indices.append(source_id - 1)
            previous = source_id
    if len(indices) != CORE_CLAUSES:
        raise FoDPOPublicationError("FoDPO mapping row count changed")
    return indices
...
def verify_mapping_clause_hashes(mapping_path: Path, cnf_path: Path) -> None:
    with mapping_path.open("r", encoding="ascii", newline="") as mapping:
        ...
        with cnf_path.open("rb") as cnf:
            header = cnf.readline()
            if header != b"p cnf 66 7686\n":
                raise FoDPOPublicationError("tracked FoDPO core header changed")
            for expected_core_id, pair in enumerate(
                zip_longest(mapping, cnf), start=1
            ):
                row, clause = pair
                if row is None or clause is None:
                    raise FoDPOPublicationError(
                        "tracked FoDPO CNF/map lengths differ"
                    )
                fields = row.rstrip("\n").split("\t")
                if int(fields[0]) != expected_core_id:
                    raise FoDPOPublicationError("tracked FoDPO map id changed")
                if hashlib.sha256(clause).hexdigest().upper() != fields[2]:
                    raise FoDPOPublicationError("tracked FoDPO clause hash mismatch")
...
def lean_module_source(indices: list[int]) -> bytes:
    if len(indices) != CORE_CLAUSES:
        raise FoDPOPublicationError("refusing Lean module with wrong index count")
    payload = f"""import LRATCatcher.Reflect
...
def foDPOCatalogueIndex : Fin 9 := ⟨{CATALOGUE_INDEX}, by decide⟩
def foDPOICNFIndex : Fin 9 := ⟨{ICNF_INDEX}, by decide⟩
...
theorem foDPOCoreIndexCount :
    foDPOCoreZeroBasedIndices.size = {CORE_CLAUSES} := by
  native_decide

theorem foDPOCoreFinIndexCount :
    foDPOCoreFinIndices.size = {CORE_CLAUSES} := by
  native_decide
...
def foDPOCoreCNFText : String :=
  include_str "../../../../scripts/r45_d12_cover9_universal/master7_r34_fodpo_core/{CORE_CNF_NAME}"
...
""".encode("utf-8")
    assert_portable(payload, "FoDPO indexed-source Lean module")
    return payload
...
def validate_external(source_dir: Path, external_core: Path) -> dict[str, object]:
    ...
    if verification.get("status") != "LEAN_REPLAY_PASS":
        raise FoDPOPublicationError("external FoDPO core lacks committed Lean replay")
    paths = external_paths(external_core)
    for name, expected in EXACT_EXTERNAL_ARTIFACTS.items():
        actual = file_metadata(paths[name])
        for field in ("bytes", "lines", "sha256"):
            if getattr(actual, field) != expected[field]:
                raise FoDPOPublicationError(f"external {name} {field} changed")
    for key, expected in (
...
def portable_reduction_payload(
    ...
    value = {
        "schema_version": 1,
        "status": "PASS_TRACKED_FODPO_CORE_REPLAY_REQUIRED",
        ...
        "formal_boundary": (
            "portable tracked copy of the exact frozen FoDPO leaf core only; "
    ...
    assert_portable(payload, "portable FoDPO reduction")
    return payload
...
def verify_portable_core() -> dict[str, object]:
    ...
    if observed not in (expected, expected | optional):
        raise FoDPOPublicationError("tracked FoDPO directory inventory is not exact")
    for name in (CORE_CNF_NAME, CORE_LRAT_NAME, MAPPING_NAME):
        ...
        for field in ("bytes", "lines", "sha256"):
            if getattr(actual, field) != frozen[field]:
                raise FoDPOPublicationError(f"tracked {name} {field} changed")
    if (TRACKED_DIRECTORY / REPLAY_NAME).read_bytes() != tracked_replay_source():
        raise FoDPOPublicationError("tracked Replay.lean changed")
    if (TRACKED_DIRECTORY / ATTRIBUTES_NAME).read_bytes() != attributes_source():
        raise FoDPOPublicationError("tracked .gitattributes changed")
    ...
    if reduction.get("status") != "PASS_TRACKED_FODPO_CORE_REPLAY_REQUIRED":
        raise FoDPOPublicationError("tracked FoDPO reduction status changed")
    if reduction.get("selection", {}).get("record") != RECORD:
        raise FoDPOPublicationError("tracked FoDPO reduction selection changed")
    claimed = reduction.get("artifacts")
    if not isinstance(claimed, dict):
        raise FoDPOPublicationError("tracked FoDPO reduction artifacts missing")
    expected_claims = {
    ...
    if claimed != expected_claims:
        raise FoDPOPublicationError("tracked FoDPO reduction artifact table changed")
    ...
    if LEAN_MODULE.read_bytes() != expected_module:
        raise FoDPOPublicationError("FoDPO indexed-source Lean module changed")
    return {
        "status": "PASS_EXACT_TRACKED_FODPO_CORE",
...
def publish(source_dir: Path, external_core: Path) -> dict[str, object]:
    for path, label in (
        (TRACKED_DIRECTORY, "tracked FoDPO directory"),
        (TRACKED_REPORT, "tracked FoDPO report"),
        (LEAN_MODULE, "FoDPO indexed-source Lean module"),
    ):
    ...
    if os.path.lexists(staging) or os.path.lexists(module_partial):
        raise FileExistsError("fresh FoDPO publication nonce unexpectedly exists")
    staging.mkdir()
    try:
        for name in (CORE_CNF_NAME, CORE_LRAT_NAME, MAPPING_NAME):
        ...
        if os.path.lexists(TRACKED_DIRECTORY) or os.path.lexists(LEAN_MODULE):
            raise FileExistsError("FoDPO publication target appeared")
...
def tracked_manifest_payload(
    ...
    value = {
        "schema_version": 1,
        "status": "PASS_TRACKED_FODPO_COMPACT_CORE_LEAN_REPLAY",
...
def milestone_payload(manifest: dict[str, object], manifest_meta: FileMeta) -> bytes:
    value = {
        "schema_version": 1,
        "status": "PASS_MASTER7_R34_FODPO_TRACKED_CORE_V1",
        "milestone": (
            "the exact FoDPO conditioned leaf has a compact tracked RUP core "
            "independently replayed by Lean/LRATCatcher"
        ),
        "artifact_directory": TRACKED_DIRECTORY.name,
        "source_fodpo_leaf": manifest["source_fodpo_leaf"],
        ...
        "value": (
            "This turns the solver-only FoDPO result into a small, portable, "
    ...
    payload = json_payload(value)
    assert_portable(payload, "tracked FoDPO milestone report")
    return payload
...
def replay_tracked(temporary_directory: Path) -> dict[str, object]:
    ...
    if any(temporary_directory.iterdir()):
        raise FoDPOPublicationError("tracked replay temporary directory is not empty")
    ...
    with locks:
        ...
        assert_portable(log_payload, "tracked FoDPO Lean log")
        ...
        success = (
            ...
            and reducer.axioms_allowed(axioms, "cover6_d7_r34_fodpo_core_unsat")
        ...
        if not success:
            raise FoDPOPublicationError("tracked FoDPO Lean replay failed closed")
...
def verify_complete() -> dict[str, object]:
    ...
    if not paths[LEAN_LOG_NAME].is_file() or not paths[MANIFEST_NAME].is_file():
        raise FoDPOPublicationError("tracked FoDPO replay publication is incomplete")
    if not TRACKED_REPORT.is_file():
        raise FoDPOPublicationError("tracked FoDPO milestone report is missing")
    for path in (paths[LEAN_LOG_NAME], paths[MANIFEST_NAME], TRACKED_REPORT):
    ...
    if manifest.get("status") != "PASS_TRACKED_FODPO_COMPACT_CORE_LEAN_REPLAY":
        raise FoDPOPublicationError("tracked FoDPO manifest status changed")
    if report.get("status") != "PASS_MASTER7_R34_FODPO_TRACKED_CORE_V1":
        raise FoDPOPublicationError("tracked FoDPO report status changed")
    ...
    if manifest.get("artifacts") != actual_artifacts:
        raise FoDPOPublicationError("tracked FoDPO manifest artifacts changed")
    if report.get("artifacts", {}).get("manifest") != file_metadata(
        paths[MANIFEST_NAME]
    ).json():
        raise FoDPOPublicationError("tracked FoDPO report manifest identity changed")
    expected_report = milestone_payload(
        manifest, file_metadata(paths[MANIFEST_NAME])
    )
    if TRACKED_REPORT.read_bytes() != expected_report:
        raise FoDPOPublicationError(
            "tracked FoDPO report differs from deterministic rendering"
        )
    return {
        **verification,
        "status": "PASS_MASTER7_R34_FODPO_TRACKED_CORE_V1",
...
def preflight() -> dict[str, object]:
    return {
        ...
        "formal_boundary": (
            "one exact FoDPO leaf core only; no semantics, S7 composition, "
