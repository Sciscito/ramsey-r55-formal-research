...
/-!
  # Selected-core semantics for the FCUj_ degree-seven branch

  This module checks only the 1,104 clauses retained by the tracked compact
  LRAT core.  Its compact payload contains one fifteen-bit orbit/lift witness
  for each of the 476 K7 clauses and 514 projected K6 clauses in core
...
open LRATCatcher.Tests.R44Cover6Master7R34FCUjCore

set_option maxRecDepth 100000

/-! ## Ordered taxonomy of the tracked core -/

def d7CoreArrayPosition (position : Fin 1104) :
    Fin fCUjCoreFinIndices.size :=
  ⟨position.val, by rw [fCUjCoreFinIndexCount]; exact position.isLt⟩

def d7CoreSourceIndex (position : Fin 1104) : Fin branchClauseCount :=
  fCUjCoreFinIndices[d7CoreArrayPosition position]

def d7SelectedCoreClause (position : Fin 1104) : CNF.Clause Nat :=
  (branchSource fCUjCatalogueIndex).clauseAt
    (d7CoreSourceIndex position)

def d7BaseCorePosition (position : Fin 98) : Fin 1104 :=
  ⟨position.val, by omega⟩

def d7RootFreeCorePosition (position : Fin 476) : Fin 1104 :=
  ⟨98 + position.val, by omega⟩

def d7RootContainingCorePosition (position : Fin 514) : Fin 1104 :=
  ⟨574 + position.val, by omega⟩

def d7UnitCorePosition (position : Fin 16) : Fin 1104 :=
  ⟨1088 + position.val, by omega⟩

theorem d7BaseCoreSourceIndex_range (position : Fin 98) :
    (d7CoreSourceIndex (d7BaseCorePosition position)).val <
      d7BaseClauseCount := by
  native_decide +revert

theorem d7RootFreeCoreSourceIndex_range (position : Fin 476) :
    d7BlockClauseStart ≤
...
theorem d7RootContainingCoreSourceIndex_range (position : Fin 514) :
    d7LocalClauseStart ≤
...
/-- Concatenated fifteen-bit codes: 476 K7 orbit witnesses, followed by
514 canonical K6 full-lift witnesses. -/
def d7CoreWitnessPayload : String :=
  include_str "../../../../scripts/r45_d12_cover9_universal/MASTER7_R34_FCUJ_CORE_SEMANTIC_WITNESSES_V1.bin"

def d7CoreWitnessChunks : Array String := #[d7CoreWitnessPayload]

def d7CoreWitnessChunkSize : Nat := 2475
...
def d7RootFreeWitnessCode (position : Fin 476) : Nat :=
  d7CoreWitnessCodeAt position.val

def d7RootContainingWitnessCode (position : Fin 514) : Nat :=
  d7CoreWitnessCodeAt (476 + position.val)

/-! ## K7 blocker reconstruction -/

def d7RootFreeSourceIndex (position : Fin 476) : Nat :=
  (d7CoreSourceIndex (d7RootFreeCorePosition position)).val

def d7RootFreeLocation (position : Fin 476) : List Nat × Nat :=
  (d7BlockLocation (d7RootFreeSourceIndex position - d7BlockClauseStart)).getD
    ([], 0)

def d7RootFreeItem (position : Fin 476) : List Nat :=
  (d7RootFreeLocation position).1

def d7RootFreeCatalogueIndex (position : Fin 476) : Nat :=
  (d7RootFreeLocation position).2

def d7RootFreeEmbedding (position : Fin 476) : MotifEmbedding :=
  fun vertex => Fin.ofNat 12 ((d7RootFreeItem position).getD vertex.val 0)

def d7RootFreeRepresentativeIndex (position : Fin 476) : Fin 6 :=
  ⟨d7RootFreeWitnessCode position % 6, Nat.mod_lt _ (by decide)⟩

def d7RootFreePermutationRank (position : Fin 476) : Fin 5040 :=
  Fin.ofNat 5040 (d7RootFreeWitnessCode position / 6)

noncomputable def d7RootFreeRepresentative (position : Fin 476) :
    RepresentativeWitness :=
  representativeWitness (d7RootFreeRepresentativeIndex position)

def d7RootFreeCube (position : Fin 476) : Cover6Cube :=
...
def D7RootFreeFiniteCheck : Prop :=
  ...
  (∀ (position : Fin 476) (left right : MotifVertex),
...
def d7RootContainingSourceIndex (position : Fin 514) : Nat :=
  (d7CoreSourceIndex (d7RootContainingCorePosition position)).val

def d7RootContainingLocation (position : Fin 514) : List Nat × Nat :=
  (d7LocalLocation
    (d7RootContainingSourceIndex position - d7LocalClauseStart)).getD ([], 0)

def d7RootContainingItem (position : Fin 514) : List Nat :=
  (d7RootContainingLocation position).1

def d7RootContainingCatalogueIndex (position : Fin 514) : Nat :=
  (d7RootContainingLocation position).2

def d7RootContainingNeighbourCount (position : Fin 514) : Nat :=
  d7NeighborCount (d7RootContainingItem position)

def d7RootContainingProjectedCube (position : Fin 514) : Cover6Cube :=
...
def d7RootContainingProjectedEmbedding (position : Fin 514) :
    MotifEmbedding :=
  fun vertex =>
    Fin.ofNat 12 ((d7RootContainingItem position).getD vertex.val 0)

def d7RootContainingEmbedding (position : Fin 514) : MotifEmbedding :=
  fun vertex =>
    if vertex.val = 0 then 0
    else Fin.ofNat 12
      ((d7RootContainingItem position).getD (vertex.val - 1) 0)

def d7RootContainingRepresentativeIndex (position : Fin 514) : Fin 6 :=
  ⟨d7RootContainingWitnessCode position % 6, Nat.mod_lt _ (by decide)⟩

def d7RootContainingPermutationRank (position : Fin 514) : Fin 5040 :=
  Fin.ofNat 5040 (d7RootContainingWitnessCode position / 6)

noncomputable def d7RootContainingRepresentative (position : Fin 514) :
    RepresentativeWitness :=
  representativeWitness (d7RootContainingRepresentativeIndex position)

def d7RootContainingFullCube (position : Fin 514) : Cover6Cube :=
...
def d7RootContainingAmbientVertex
    (position : Fin 514) (vertex : MotifVertex) : Nat :=
  (d7RootContainingItem position).getD (vertex.val - 1) 0
...
def D7RootContainingFiniteCheck : Prop :=
  ...
  (∀ (position : Fin 514) (left right : MotifVertex),
  ...
  (∀ (position : Fin 514) (vertex : MotifVertex), 0 < vertex.val →
  ...
  (∀ (position : Fin 514) (right : MotifVertex), 0 < right.val →
    (d7RootContainingFullCube position).fixed.testBit
...
theorem d7RootFreeEmbedding_injective (position : Fin 476) :
    Function.Injective (d7RootFreeEmbedding position) :=
  d7RootFreeFiniteChecks.2.2.1 position

theorem d7RootFreeClause_eq (position : Fin 476) :
    d7SelectedCoreClause (d7RootFreeCorePosition position) =
      cubeDimacsBlocker (d7RootFreeEmbedding position)
        (d7RootFreeCube position) :=
  d7RootFreeFiniteChecks.2.2.2 position

theorem d7RootFreeCube_orbit (position : Fin 476) :
...
noncomputable def d7RootFreeBlockerWitness (position : Fin 476) :
...
theorem d7RootContainingProjectedClause_eq (position : Fin 514) :
    d7SelectedCoreClause (d7RootContainingCorePosition position) =
      cubeDimacsBlocker (d7RootContainingProjectedEmbedding position)
        (d7RootContainingProjectedCube position) :=
  d7RootContainingFiniteChecks.2.1 position

theorem d7RootContainingEmbedding_injective (position : Fin 514) :
    Function.Injective (d7RootContainingEmbedding position) :=
  d7RootContainingFiniteChecks.2.2.1 position

theorem d7RootContainingFullCube_orbit (position : Fin 514) :
...
theorem d7RootContainingEmbedding_nonroot
    (position : Fin 514) (vertex : MotifVertex)
...
theorem d7RootContainingEmbedding_root_val (position : Fin 514) :
    (d7RootContainingEmbedding position 0).val = 0 :=
...
theorem d7RootContaining_root_bit
    (position : Fin 514) (right : MotifVertex)
    (hright : 0 < right.val)
    (hfixed : (d7RootContainingFullCube position).fixed.testBit
      (graph6EdgePosition 0 right.val) = true) :
    (d7RootContainingFullCube position).ones.testBit
...
theorem d7SelectedBaseClause_eq (position : Fin 98) :
...
theorem d7SelectedCoreClause_eval_true
  ...
  by_cases hbase : position.val < 98
  · let basePosition : Fin 98 := ⟨position.val, hbase⟩
  ...
  · by_cases hrootFree : position.val < 574
    · let payloadPosition : Fin 476 :=
        ⟨position.val - 98, by omega⟩
    ...
    · by_cases hrootContaining : position.val < 1088
      · let payloadPosition : Fin 514 :=
          ⟨position.val - 574, by omega⟩
        have hposition :
        ...
        exact rootContainingBlockerWitness_eval_true hfree havoid
          (d7RootContainingBlockerWitness coloring hrootPositive hrootRight
            payloadPosition)
      · let unitPosition : Fin 16 := ⟨position.val - 1088, by omega⟩
...
theorem d7CoreSelection_eval_true
    ...
    (hunit : ∀ position : Fin 16,
      CNF.Clause.eval coloring
        (d7SelectedCoreClause (d7UnitCorePosition position)) = true) :
    CNF.eval coloring
      ((branchSource fCUjCatalogueIndex).selectCNF
        fCUjCoreFinIndices) = true := by
  rw [CNF.eval, Array.all_eq_true]
  intro index hindex
  have hsize :
      ((branchSource fCUjCatalogueIndex).selectCNF
        fCUjCoreFinIndices).clauses.size = 1104 := by
    unfold LRATCatcher.Tests.R44Cover6Master8CoreBridge.IndexedCNFSource.selectCNF
    simpa using fCUjCoreFinIndexCount
  let position : Fin 1104 := ⟨index, by omega⟩
...
theorem no_degreeSeven_fCUj_cover6_avoiding
  ...
  have hunit : ∀ position : Fin 16,
    ...
    exact fCUjRelabeled_unitTail_eval_true coloring
  ...
  have hfalse := fCUjCoreSelection_unsat normalized
...
#print axioms no_degreeSeven_fCUj_cover6_avoiding

end LRATCatcher.Tests.R44Cover6Master7R34FCUjSemantics
